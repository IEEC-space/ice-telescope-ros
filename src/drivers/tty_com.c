...
int tty_read(int fd, char *buf, int nbytes, int timeout, int *nbytes_read)
{
	...
	*nbytes_read =0;
	...
	if (tty_debug)
		fprintf(stderr, "%s: Request to read %d bytes with %d seconds timeout for fd %d\n", __FUNCTION__, nbytes, timeout, fd);
...
int tty_read_section(int fd, char *buf, char stop_char, int timeout, int *nbytes_read)
{
	...
	*nbytes_read = 0;
	...
	if (tty_debug)
		fprintf(stderr, "%s: Request to read until stop char '%c' with %d seconds timeout for fd %d\n", __FUNCTION__, stop_char, timeout, fd);
