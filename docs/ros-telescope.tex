...
\section{Description}
%%%%%%%%%%%%%%%%%%%%%
\Prog{ice\_telescope} is composed of several nodes --\Prog{ice\_telescope\_node}-- that allow the control of the telescope system. In addition to the server to control all devices \Prog{ice\_tel\_server}, each of the system components (telescope, dome, ccd, pdu, weather station) has a client node following the naming convention \Prog{brand\_client}: 
\begin{Description}[\Opt{Full-Server}]\setlength{\itemsep}{0cm}
\item[\Opt{Full-Server}] \Prog{ice\_tel\_server}.
\item[\Opt{Telescope}] \Prog{meade\_client}.
\item[\Opt{CCD}] \Prog{sbig\_client}.
\item[\Opt{Dome}] \Prog{baader\_client}.
\item[\Opt{PDU}] \Prog{apc\_client}.
\item[\Opt{WS}]	\Prog{vaisala\_client}.
\end{Description}
...
\item[\Arg{weather\_station}] The specified \Arg{action} will be performed on the weather station.

	\Prog{rosrun} \Prog{ice\_telescope} \Prog{apc\_client} \Arg{action} \Arg{weather\_station}
